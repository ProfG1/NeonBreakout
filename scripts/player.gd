...
func _process(_delta):
	...
	#jump
	if velocity.y < 1 and jump and !dashing:
		anim.speed_scale = 1
		anim.animation = "jump"
	...
	if latch and slide:
		...
		if crouching and !rolling:
			if abs(velocity.x) > 10:
				anim.speed_scale = 1
				anim.animation = "crouch_walk"
			else:
				anim.speed_scale = 1
				anim.animation = "crouch_idle"
...
func _on_player_sprite_sprite_frames_changed():
	pass # Replace with function body.
